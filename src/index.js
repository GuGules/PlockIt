...
import { getLatestCampaignFile } from "./api/modules/campaign.js";
...
app.use((req, res, next) => {
    if (config.security.secured_mode && !config.security.authorized_ips.includes(req.ip) && !config.security.temporary_authorized_ip.some((ip) => { ip.ip == req.ip })) {
        console.log(chalk.red(`[${new Date().toISOString()} - ${req.ip}] ${req.method} - ${req.path}`))
...
app.get('/err', (req, res) => {
    return res.sendFile(__dirname + '/front/errors/500.html')
})

app.get('/credits', (req, res) => {
    return res.sendFile(__dirname + '/front/credits.html');
});

app.get('/thanks', (req, res) => {
    return res.sendFile(__dirname + '/front/thanks.html');
});

app.get('/ipmanager', (req, res) => {
    return res.sendFile(__dirname + '/front/ipmanager.html');
})

app.get('/col/:id', (req, res) => {
    if (req.params.id == "last") {
        return res.redirect(`/col/${getLatestCampaignFile()}`);
    }
    return res.sendFile(__dirname + "/front/col.html");
})
...
app.get('/vote/:id', (req, res) => {
    ...
    if (req.params.id == "last") {
        return res.redirect(`/vote/${getLatestCampaignFile()}`);
    }
    return res.sendFile(__dirname + '/front/vote.html');
});

app.get('/contribute/:id', (req, res) => {
    if (req.params.id == "last") {
        return res.redirect(`/contribute/${getLatestCampaignFile()}`);
    }
    return res.sendFile(__dirname + '/front/contribute.html');
});

app.get('/rapport/:id', (req, res) => {
    if (req.params.id == "last") {
        return res.redirect(`/rapport/${getLatestCampaignFile()}`);
    }
    return res.sendFile(__dirname + '/front/rapport.html');
})

if (config.environment == "dev") {
    app.get('/errors/:id', (req, res) => {
        const id = req.params.id;
        return res.sendFile(__dirname + `/front/errors/${id}.html`)
    })
}
...
app.get('/:id', (req, res) => {
    ...
    if (req.params.id == "last") {
        return res.redirect(`/${getLatestCampaignFile()}`);
    }
